##https://certara.github.io/R-RsNLME/articles/simulation.html
#https://github.com/certara/tidyvpc
#Sys.setenv(R_REMOTES_NO_ERRORS_FROM_WARNINGS="true")
# install dev version of tidyvpc
#remotes::install_github("certara/tidyvpc")
## always sort sim dat by rep,id, time
rm(list=ls())
home_dir <-  "c:/git/dual_numbers/pydarwin_nlme/run/0"  
nreps <- 200
library(stringr)
library(dplyr)
...
library(tidyvpc) 
library(lubridate)
ETANOMEGA <- c(1,2,3,4,3,6) # no clear way to get n_omega from fit object
Results <- data.frame(
  ...
  for(this_vwt in 0:1){
    for(this_comp in 0:1){
      for(this_eta in 0:5){
        ...
        message("############ Running ",model_orig@modelInfo@workingDir, " at ", as.ITime(Sys.time())," ############")
        ...
        if(file.exists(file.path(model_orig@modelInfo@workingDir,"err2.txt"))){
          ...
          CurResults <- data.frame(
            ...
            message("############ Running NPDE for ", model_orig@modelInfo@workingDir," at ",as.ITime(Sys.time()) , " ############")
...
write.csv(Results,"c:/git/dual_numbers/CompileResults/NLMEStdResults.csv",quote= FALSE, row.names = FALSE)
message("Done at ", as.ITime(Sys.time()))